...
void Partition::SetPartition(const PreParamGridWell& grid)
{
	...
	if (myrank == MASTER_PROCESS) {
		...
		const int rest = numElementTotal % numproc;
	...
	if (myrank == MASTER_PROCESS) {
		...
		numEdges    = new OCP_INT[numproc]();
		maxNumVtx   = 0;
		maxNumEdges = 0;
		for (OCP_INT p = 0; p < numproc; p++) {			
			for (OCP_INT n = vtxdist[p]; n < vtxdist[p + 1]; n++) {
				numEdges[p] += grid.numNeighbor[n];				
			}
			maxNumVtx   = max(maxNumVtx, (OCP_INT)(vtxdist[p + 1] - vtxdist[p]));
			maxNumEdges = max(maxNumEdges, numEdges[p]);			
		}
	}

	MPI_Scatter(numEdges, 1, MPI_INT, &numEdgesLocal, 1, MPI_INT, MASTER_PROCESS, myComm);
	MPI_Status status;
	...
	if (myrank == MASTER_PROCESS) {
		...
		for (OCP_INT p = 1; p < numproc; p++) {
			...
			for (OCP_INT n = 0; n < vtxdist[p + 1] - vtxdist[p]; n++) {
		...
		for (OCP_INT n = 0; n < numElementLocal; n++) {
...
void Partition::SetDistribution()
{
	...
	OCP_INT*    adjproc = new OCP_INT[numEdgesLocal]();
	...
	for (OCP_INT i = 0; i < numElementLocal; i++) {
		for (OCP_INT j = xadj[i]; j < xadj[i + 1]; j++) {
			if (adjncy[j] < vtxdist[myrank] || adjncy[j] >= vtxdist[myrank + 1]) {
				// adjncy[j] is not in current proc, find its process
				// estimate first
				OCP_INT p = adjncy[j] / numElementLocal;  // p must be an signed int
	...
	for (OCP_INT i = 0; i < right_neighbor.size(); i++) {
	...
	for (OCP_INT n = 0; n < right_neighbor.size(); n++) {
		for (OCP_INT i = 1; i < right_neighbor[n].size(); i++) {
	...
	for (OCP_INT i = 0; i < numElementLocal; i++) {
		...
		if (part[i] == myrank) {
			...
			for (OCP_INT s = 0; s < send_buffer.size(); s++) {
	...
	vector<OCP_INT> send_size(numproc, 0);
	vector<OCP_INT> recv_size(numproc, 0);
	for (const auto& s : send_buffer) {
		send_size[s[0]] = s.size();
	}

	MPI_Alltoall(&send_size[0], 1, MPI_INT, &recv_size[0], 1, MPI_INT, myComm);
	...
	// Free Memory
	vector<OCP_INT>().swap(send_size);
	vector<OCP_INT>().swap(recv_size);
