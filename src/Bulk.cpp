...
void Bulk::InputRockFuncT(const ParamReservoir& rs_param)
{
    for (USI i = 0; i < NTROCC; i++) {
        if (rs_param.rockSet[i].type == "LINEAR") {
            rock.push_back(new OCPRockT_Linear(rs_param.rockSet[i]));
        } else {
            rock.push_back(new OCPRockT_Exp(rs_param.rockSet[i]));
        }
    }
...
void Bulk::InitPTSw(const USI& tabrow)
{
    ...
    if (Dref < DOGC) {
        ...
        gammaGtmp = GRAVITY_FACTOR *
                    flashCal[0]->RhoPhase(Pgref, Pbb, myTemp, tmpInitZi, GAS);
        ...
        for (USI i = 0; i < mynum; i++) {
            ...
            gammaGtmp = GRAVITY_FACTOR *
                        flashCal[0]->RhoPhase(Ptmp, Pbb, myTemp, tmpInitZi, GAS);
            Ptmp += gammaGtmp * mydz;
            myz += mydz;
        }
        Ptmp -= PcGO;
        for (USI i = 0; i < mynum; i++) {
            ...
            gammaOtmp = GRAVITY_FACTOR *
                        flashCal[0]->RhoPhase(Ptmp, Pbb, myTemp, tmpInitZi, OIL);
            Ptmp -= gammaOtmp * mydz;
            myz -= mydz;
        }
        Poref = Ptmp;
        ...
        gammaOtmp = GRAVITY_FACTOR *
                    flashCal[0]->RhoPhase(Poref, Pbb, myTemp, tmpInitZi, OIL);
        ...
        gammaWtmp = GRAVITY_FACTOR *
                    flashCal[0]->RhoPhase(Pwref, Pbb, myTemp, tmpInitZi, WATER);
...
OCP_DBL Bulk::CalFPR(OCP_DBL& vtmp) const
{
    ...
    if (numPhase == 3) {
        for (OCP_USI n = 0; n < numBulkInterior; n++) {
